...
public class BicicletaApplication {
	...
	public CommandLineRunner demo(IClienteDAO clienteDAO, BCryptPasswordEncoder encoder, ILocadoraDAO locadoraDAO, ILocacaoDAO locacaoDAO) {
		return (args) -> {
			...
			locacao.setLocadora(l1);
			LocalDateTime dataHora = LocalDateTime.now();
			String dataString = dataHora.toString();
			dataString = dataHora.format(DateTimeFormatter.ofPattern("dd/MM/yyyy HH:mm:ss"));
			locacao.setData(dataString);
			locacaoDAO.save(locacao);

			Locacao locacao2 = new Locacao();
			locacao2.setCliente(cliente);
			locacao2.setLocadora(l1);
			dataHora = LocalDateTime.now();
			dataString = dataHora.toString();
			dataString = dataHora.format(DateTimeFormatter.ofPattern("dd/MM/yyyy HH:mm:ss"));
			locacao.setData(dataString);
